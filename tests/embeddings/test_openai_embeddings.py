...
def test_embed_default_model(mock_openai_client):
    ...
    mock_openai_client.embeddings.create.assert_called_once_with(
        input=["Hello world"], model="text-embedding-3-small", dimensions=1024
    )
...
def test_embed_without_api_key_env_var(mock_openai_client):
    ...
    mock_openai_client.embeddings.create.assert_called_once_with(
        input=["Testing API key"], model="text-embedding-3-small", dimensions=1024
    )
...
def test_embed_uses_environment_api_key(mock_openai_client, monkeypatch):
    ...
    mock_openai_client.embeddings.create.assert_called_once_with(
        input=["Environment key test"], model="text-embedding-3-small", dimensions=1024
    )
